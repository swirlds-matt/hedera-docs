...
> _**Note: This tutorial requires the use of the following tools: Hedera**_ [_**JavaScript SDK**_](../../sdks-and-apis/sdks/)_**, Solidity with libraries for random number generation (**_[_**HIP-351**_](https://hips.hedera.com/hip/hip-351)***), and the Mirror Node REST API (**_[_**learn more**_](https://hedera.com/blog/how-to-look-up-transaction-history-on-hedera-using-mirror-nodes-back-to-the-basics)***) and explorer (**_[_**HashScan**_](https://hashscan.io/#/mainnet/dashboard)***).**
...
**Step 2:** Deploy a Solidity smart contract to generate the random number. **Step 2:** Deploy a Solidity smart contract to generate the random number. After completing all steps, your console should look something like [this](how-to-generate-a-random-number-on-hedera.md#console-output-1).
...
* Use the helper _**contracts.executeContractFcn**_ to execute the contract function _**getPseudorandomNumber**_
  * Use **ContractFunctionParameters()** from the SDK to specify the parameters for the contract function (_**randNumParams**_). Pass the lower (_**lo**_) and upper (_**hi**_) limits for the random number Pass the lower (_**lo**_) and upper (_**hi**_) limits for the random number
  ...
  * The contract function calls a precompiled contract and gets the bytes for the random seed. The contract function calls a precompiled contract and gets the bytes for the random seed. The random number is calculated, stored in the state variable _**randNum**_, and returned by the function
...
You will learn various ways to obtain the random number from the Solidity contract. The best approach depends on your use case and preference. You will learn various ways to obtain the random number from the Solidity contract. The best approach depends on your use case and preference. You can get the random number by: using a transaction record, doing a contract call to read state variables, and checking a mirror node explorer.
...
* Use the helper function _**queries.txRecQueryFcn**_ to obtain information from the transaction record
  ...
  * The query of the transaction record, _**recQuery**_, was configured to return information on child transactions. Thus, the first child transaction of the contract execution (parent transaction) contains information about the bytes needed for the random number – see _**recQuery.children\[0].prngBytes**_ Thus, the first child transaction of the contract execution (parent transaction) contains information about the bytes needed for the random number – see _**recQuery.children\[0].prngBytes**_
...
====================================================\n`);
} console.log(`- The random number checks out ✅`) : console.log(`- Random number doesn't match ❌`);
...
**Congratulations! Congratulations! 🎉 You have successfully learned how to generate a random number on Hedera. Feel free to reach out if you have any questions: Feel free to reach out if you have any questions:**
